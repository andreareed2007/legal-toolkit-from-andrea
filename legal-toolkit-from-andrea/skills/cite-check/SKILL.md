...
> **Version:** v2026.08.04-3 (shared edition) · **Last updated:** 2026.08.05
...
## HARD RULE: Authoritative Scripts Only (2026.08.03)

**The scripts bundled in this skill's `scripts/` folder are the authoritative copies for this edition — always run them from there.** If a copy of these scripts exists anywhere else on the machine, do not mix the two: a stale copy once produced four false flags from bugs already fixed upstream. Before Step 2, run this sanity check from the scripts folder you are about to use and STOP if it fails:

```bash
python3 -c "
import cc_proposition, inspect
src = inspect.getsource(cc_proposition.extract)
assert 'is_id' in src, 'STALE SCRIPTS: missing Session D Fix 6 (is_id) - you are NOT on the authoritative source'
assert '\u2026' not in dict(cc_proposition._SMART), 'STALE SCRIPTS: _SMART still folds the ellipsis - pre-2026.08.03 copy'
print('script provenance OK')"
```
...
Verify imports before running anything (from the scripts folder):
...
If this fails, STOP and report the actual traceback. eyecite is HARD-pinned at 2.7.8 (validated; see ENVIRONMENT_SETUP.md). Upgrading eyecite is a deliberate act: bump the pin, then run pytest + all six gates BEFORE any real brief.
...
ELSE:                  the folder the brief came from (or the session outputs folder)/YYYY.MM.DD Cite Check Report - [Brief Name].{ext}
```
...
0. **PRE-PRESENT CARD SCAN (2026.08.04, mandatory — a live-run lesson).** Read EVERY problem-tab card against the brief text before presenting; a CRITICAL card that has not been individually read is not presentable. HARD STOPS — send the report back to diagnosis, never present, if: (a) any citation string or pincite carries a record fingerprint (¶, page:line like `28:21-23`, Apx./App'x, Ex., Dep.) — that is a record cite that leaked into the case pipeline; (b) any card headline names a case that does not appear in the brief text (wrong-case caption leak); (c) an UNABLE card cites a state intermediate appellate case by docket + WL number — those are on CourtListener far more often than not, so treat a cluster of such UNABLEs as a resolver defect, not a coverage gap. Validating a subset of target cards NEVER substitutes for this scan.
...
- **Footnotes (2026.07.04, do not regress):** brief-side — `find_footnotes` blocks END at the paragraph break (the old next-footnote-only rule swallowed all following body text), and `foots` MUST be passed to `extract()` on every path (it was silently dead on the live path — a live wrong-proposition card). Case-side — a pincite carrying `n.N` triggers the endnote-tail append in `_trim_to_pincite` (CL renders footnotes AFTER the body; the page window otherwise excludes the cited text — a live false "Does Not Support"); the endnote block is found by its own lead pattern, never by the last star marker (footnote citations carry star-pincites of their own). The report renders "Supporting passage located: in the opinion's footnotes" for passages behind the sentinel.
...
- **The regression gates** (`cc_prop_gate.py`, `cc_resolver_gate.py`, `cc_verify_gate.py`, AND `cc_application_gate.py`) run before AND after every code change; cc_prop_gate's ANSWER-KEY line is a metric (extractor-only — the live hybrid number is higher by design; do NOT chase the gate metric with extractor hacks). cc_resolver_gate replays a recorded cassette offline (no token, ~5s); a code change that alters the API call sequence exits loudly — re-record with `--record` (resumable windows) and re-pin improvements with `--rebaseline`. Unit tests: `python3 -m pytest tests/ -q`.
...
- **Step 6.6 must-verify loop (2026.07.10, do not regress):** auto-resolved adverse verdicts on unpublished NY slip ops / free-source copies are as likely resolution artifacts as real defects — the loop exists because a 2026.07.09 live run shipped three false-negative slip-op verdicts that a human caught only post-delivery. The render hard-block is the point: never bypass it by editing the manifest by hand, and never grant the override without the fetched text passing the identity gate + VERBATIM quote match. The manifest preserves worked findings across phase2 re-runs (`_apply_verification`) — a re-run must never silently discard agent verification work. Gate: `cc_verify_gate.py` (run before AND after every change to the trigger predicate or verdict layer, with the other gates).
- **Goodlaw (2026.07.06, additive — do not couple):** verify() and the verdict taxonomy are UNTOUCHED; Treatment is its own axis, own module (`cc_goodlaw.py`), own state file (`cc_goodlaw.pkl`), own gate (`cc_goodlaw_gate.py` — run before AND after every goodlaw change, plus cc_prop_gate + pytest). A hollow scan (fetch failure, empty cluster) must land NOT CHECKED, never "no signal" (the hollow-scan fallback regression). The four scan guards exist because the good-law CONTROL fixture false-positived without them — never remove one without re-running the gate. `build` clears cc_goodlaw.pkl with the rest of downstream state. Without goodlaw state, render output is unchanged (additive by construction).

- **Application-sentence layer (2026.08.04, locked design — do not regress):** `cc_application.py` detects propositions that APPLY the cited rule to the instant facts (roster/role-noun/artifact Tier 1 + marker-family Tier 2; fire rule (T1 AND T2) or 2 T2 families with cited-case-description and generic-subject guards; "as a matter of law" alone never fires; `parenthetical`/`citing_parenthetical` kinds exempt; role nouns capitalized-only; roster names colliding with TOA or detected authority captions are downweighted, word-bounded). Re-render engages ONLY where the machine verdict would land does-not-support or weak-REVIEW, through ONE shared decision point (`cc_application.rerender_key`, used by BOTH `cc_severity.check_support` and `cite_check_report._verdict` — never fork them): verified sibling (score ≥0.8 / verbatim quote / Step 6.6 override on the SAME resolved authority) → "APPLIED RULE — VERIFIED ELSEWHERE" (Tier 4 REVIEW, never PASS; card names the sibling + pincite equality); characterization predicates ("misplaced," "inapposite," "reliance on X is misplaced/misleading") → "CITED TO DISTINGUISH" (Tier 4); NO sibling → verdict stays DOES NOT SUPPORT with an additive note (bad cites are never masked). SUPPORTED outcomes and the quote override are never touched. Roster is harvested ONCE in `build_citations` from the PRE-strip text (the caption lives in stripped front matter); `attach()` runs in `finalize_results` and again at `render` (idempotent — Step 6.6 overrides update siblings). Gate: `cc_application_gate.py` replays the labeled dev set (`application_devset.json`) offline — precision on rule sentences must stay 1.00 (a false fire downgrades a real DNS); recall is precision-limited by design (quote-carrying application sentences ride the verbatim override instead). Validated live on a 41-instance MSJ run (2026.08.04): exactly one APPLIED RULE card (a short-form cite whose sibling was the full cite at the same pincite); every other card untouched.
- **Session A accuracy fixes (v2026.07.28-1, doubled-layer-PDF diagnosis, do not regress):** confirmation of a FABRICATED quote now requires a COMPLETENESS ATTESTATION (`cite_check._opinion_is_complete` on the stored copy, or `full_text_complete` recorded by `patch_gap`), NOT `len(full_text) >= len(opinion_text)` — a length comparison cannot detect truncation and shipped three false CRITICALs. An unresolved `id.` carrying a quote is RETAINED (attached to the nearest preceding authority, review-flagged), but an `id. §`/`id., Ex.` (statute/contract/record) is NOT rescued as a case. An `id.` after a `(quoting/citing …)` parenthetical chains to the parenthetical PARENT (Bluebook R4.1), and an `id.` TERMINATES its sentence (proposition = preceding sentence). Quote marks pair in DOCUMENT ORDER (shared `_paired_quote_spans`) — never re-pair a closer with the next opener. A quoted span belongs only to the cite immediately after its closing mark; string-cite/see-also members are graded support-only (`quote_support_only`), never FABRICATED for another authority's quote. The TOA heading regex matches `(TABLE|INDEX) OF (AUTHORITIES|CITATIONS)`/`AUTHORITIES CITED` with a dotted-leader structural fallback. Report panes stay in the browser-searchable flow (no `display:none` on `.tabpane`). `cc_verify_gate` encodes the completeness-attestation contract; all five gates green.
